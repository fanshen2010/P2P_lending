...
<#list rstlist?keys as key>
	<span class=" check_content uniformjs">
    	<label class="frm_checkbox_label selected">
            <input  id="" name="${(name)!}" class="frm_checkbox" type="checkbox"  name="" value="${key}" <#if disabled?? && disabled>disabled=disabled</#if> <#if checkedList?seq_contains("${key}")>checked="checked"</#if> />${rstlist[key]}
        </label>
    </span>
</#list>
</#if>