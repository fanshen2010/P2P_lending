												...
												c
														.addEvent(
																...
																function(s) {
																	...
																	s
																			.preventDefault();
																	no_type_restriction: for (p = 0; p < n.filters.length; p++) {
					...
					"flash",
					{
						...
						init : function(k, p) {
							...
							j.innerHTML = '<object id="'
									...
									+ l
									+ '" /><param name="wmode" value="transparent" /><param name="allowscriptaccess" value=window.location.href /></object>';
							function n() {
													...
													a
															.addEvent(
																	...
																	function(w) {
																		...
																		w
																				.preventDefault();
																		no_type_restriction: for (s = 0; s < v.length; s++) {
																			t = v[s].extensions
																					.split(",");
			...
			.addRuntime(
					"html5",
					{
						getFeatures : function() {
							var t, o, s, r, q, n, p;
							p = (function() {
								var y = navigator, x = y.userAgent, z = y.vendor, v, u, w;
								v = /WebKit/.test(x);
								w = v && z.indexOf("Apple") !== -1;
								u = h.opera && h.opera.buildNumber;
								return {
									ie : !v && !u && (/MSIE/gi).test(x)
											&& (/Explorer/gi).test(y.appName),
									webkit : v,
									gecko : !v && /Gecko/.test(x),
									safari : w,
									safariwin : w
											&& navigator.platform
													.indexOf("Win") !== -1,
							...
							if (h.XMLHttpRequest) {
								...
								s = !!t.upload;
								o = !!(t.sendAsBinary || t.upload)
							}
							if (o) {
								r = !!(t.sendAsBinary || (h.Uint8Array && h.ArrayBuffer));
								q = !!(File
										&& (File.prototype.getAsDataURL || h.FileReader) && r);
							...
							return {
								...
								dragdrop : (function() {
									...
									return ("draggable" in u)
											|| ("ondragstart" in u && "ondrop" in u)
								}()),
								jpgresize : q,
								pngresize : q,
								multipart : q || !!h.FileReader || !!h.FormData,
								canSendBinary : r,
								cantSendBlobInFormData : !!(p.gecko
										&& h.FormData && h.FileReader && !FileReader.prototype.readAsArrayBuffer),
								progress : s,
								chunks : n,
								triggerDialog : (p.gecko && h.FormData || p.webkit)
							}
						},
						init : function(p, q) {
							var n;
							function o(v) {
								...
								for (s = 0; s < v.length; s++) {
									...
									w = j.guid();
									c[w] = t;
									u.push(new j.File(w, t.fileName || t.name,
											t.fileSize || t.size))
								}
								if (u.length) {
							...
							p
									.bind(
											...
											function(v) {
												...
												if (E) {
													var x = v.settings.browse_button_hover, z = v.settings.browse_button_active, w = v.features.triggerDialog ? E
															: F;
													if (x) {
														j.addEvent(w,
																"mouseover",
											...
											function(r, t) {
												...
												function w(z, C, y) {
													...
													if (File.prototype.slice) {
														...
														if (A = File.prototype.webkitSlice
												...
												function v(z) {
													var C = 0, B = 0, y = ("FileReader" in h) ? new FileReader
															: null, D = typeof (z) === "string";
													function A() {
														var I, M, K, L, H, J, F, E = r.settings.url;
														function G(W) {
															var T = 0, U = new XMLHttpRequest, X = U.upload, N = "----pluploadboundary"
																	+ j.guid(), O, P = "--", V = "\r\n", R = "";
															if (X) {
	...
	function a() {
		...
		return {
			...
			EXIF : function() {
				...
				if (u.ExifVersion) {
					u.ExifVersion = String.fromCharCode(u.ExifVersion[0],
							u.ExifVersion[1], u.ExifVersion[2],
							u.ExifVersion[3])
				}
				return u
			},
	...
	b.runtimes.Html4 = b
			.addRuntime(
					"html4",
					{
						getFeatures : function() {
							var f = (function() {
								var l = navigator, k = l.userAgent, m = l.vendor, h, g, j;
								h = /WebKit/.test(k);
								j = h && m.indexOf("Apple") !== -1;
								g = d.opera && d.opera.buildNumber;
								return {
									ie : !h && !g && (/MSIE/gi).test(k)
											&& (/Explorer/gi).test(l.appName),
									webkit : h,
									gecko : !h && /Gecko/.test(k),
									safari : j,
									safariwin : j
											&& navigator.platform
													.indexOf("Win") !== -1,
							...
							return {
								multipart : true,
								triggerDialog : (f.gecko && d.FormData || f.webkit)
							}
						},
						init : function(f, g) {
							f
